...
def main():
    ...
    chosen = np.r_[np.concatenate([rng.choice(indices,60) for indices in val_positive]),rng.choice(np.flatnonzero(~vl),240)]
    ...
    config = {**vars(args), 'graph_sha256':sha(Path(args.graph)/'graph.npz'), 'fields':FIELDS,
              ...
              'sampling':'Half positive; positive task kinds balanced equally, including short green-light beeps',
    ...
    for step in range(1,args.steps+1):
        ...
        indices=np.r_[pos,rng.choice(negative,args.batch-len(pos))]
