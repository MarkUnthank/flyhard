...
def main():
    ...
    def validate():
        total=0.
        with torch.no_grad():
            for i in range(0,len(vx),args.batch):total+=float(((policy(vx[i:i+args.batch])-vy[i:i+args.batch])/policy.scale).square().sum())
        return total/(2*len(vx))
    ...
    for step in range(1,args.steps+1):
        ...
        loss=((policy(tx[idx])-ty[idx])/policy.scale).square().mean();loss.backward()
        if gradient is None:
