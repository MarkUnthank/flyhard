...
def main():
    ...
    p.add_argument('--edit', default='configs/roundabout-smooth25s.json')
    ...
    edit = json.loads(Path(args.edit).read_text())
    assert edit['fps'] == 60
    timeline = []
    for shot in edit['shots']:
        count = round(shot['seconds'] * 60)
        start, end = shot['source_start'], shot['source_end']
        assert 0 <= start < end <= len(frames)-1
        timeline.extend({**shot, 'source_index': start + j / count * (end - start)} for j in range(count))
    credit_frames = round(edit['credit_seconds'] * 60)
    first_source = timeline[0]['source_index']
    assert int(first_source) == first_source
    ...
    if 'rear_lamp' in edit['cameras']:
        for geom in range(rig.model.ngeom):
            name = mj.mj_id2name(rig.model, mj.mjtObj.mjOBJ_BODY, rig.model.geom_bodyid[geom]) or ''
            if name.startswith('nmf/rf_'):
                rig.model.geom_matid[geom] = -1
                rig.model.geom_rgba[geom] = [.85, .85, .85, 1.]
    ...
    # A fresh episode resets the replayer clock between repeated render jobs.
    ...
    for _ in range(max(1, int(first_source)-2)): world.tick()
    ego = next(a for a in world.get_actors().filter('vehicle.*') if a.attributes.get('role_name') == 'hero')
    previous_source = float(first_source)
    client.set_replayer_time_factor(0.)
    settings.fixed_delta_seconds = 1 / 60; settings.no_rendering_mode = False
    world.apply_settings(settings)
    # Resetting a live CARLA replayer can leave a one-frame phase difference.
    # Anchor the start against the saved moving trajectory before creating shots.
    trajectory = np.asarray([f['vehicle_matrix'] for f in frames])[:, :3, 3]
    start_alignment = []
    for _ in range(6):
        for __ in range(2): world.tick()
        position = np.asarray(ego.get_transform().get_matrix())[:3, 3]
        error = float(np.linalg.norm(position - trajectory[int(first_source)]))
        if error < .01: break
        candidates = []
        for k in range(max(0, int(first_source)-8), int(first_source)+2):
            direction = trajectory[k+1] - trajectory[k]
            fraction = np.clip(np.dot(position-trajectory[k], direction) / max(np.dot(direction,direction),1e-12),0,1)
            distance = np.linalg.norm(position - trajectory[k] - fraction*direction)
            candidates.append((distance, k+fraction))
        _, observed_source = min(candidates)
        remaining = first_source - observed_source
        start_alignment.append({'observed_source':float(observed_source),'error_m':error})
        assert remaining > 0, ('Replay started ahead of requested source', start_alignment)
        if remaining > 1:
            client.set_replayer_time_factor((remaining-1)*3); world.tick()
            remaining = 1.
        client.set_replayer_time_factor(remaining*3); world.tick()
        client.set_replayer_time_factor(0.)
    assert error < .01, ('Could not align replay start', start_alignment)
    print(json.dumps({'start_alignment':start_alignment,'aligned_error_m':error}),flush=True)
    ...
    try:
        for output_index, shot in enumerate(timeline):
            ...
            if name in edit['cameras']:
                setup = edit['cameras'][name]
                pose, fov = look_at(setup['position'], setup['target']), setup['fov']
            elif name == 'orbit': pose, fov = orbit_pose(source * .05), 65
            ...
            if name != last_camera:
                ...
                for kind in ['rgb', 'depth']:
                    ...
                    bp.set_attribute('lens_k', '0.0'); bp.set_attribute('lens_kcube', '0.0')
            ...
            native_lights = int(ego.get_light_state())
            native_signal = 'right' if native_lights & 16 else 'left' if native_lights & 32 else 'off'
            lamp_roi = raw[370:570, 440:830]
            amber_pixels = int(np.sum((lamp_roi[:,:,0] > 200) & (lamp_roi[:,:,1] > 130) & (lamp_roi[:,:,2] < 100)))
            bright_lamp_pixels = int(np.sum((lamp_roi[:,:,0] > 245) & (lamp_roi[:,:,1] > 235) & (lamp_roi[:,:,2] > 180)))
            ...
            assert error < .12, (output_index, source, error)
            if native_video:
            ...
            if 'body_camera' in shot:
                bc = shot['body_camera']
                body_camera.lookat[:] = rig.stalk.center + bc['offset']
                body_camera.distance = bc['distance']; body_camera.azimuth = bc['azimuth']; body_camera.elevation = bc['elevation']
            elif shot['body_closeup']:
            ...
            title = shot['title']
            if native_signal == 'right': title = shot.get('title_after_signal_on', title)
            if native_signal == 'off': title = shot.get('title_after_signal_off', title)
            draw.text((24,23),title,font=fonts[24],fill='#eee')
            ...
            records.append({'output_frame':output_index,'output_time':output_index/60,'source_index':source,
                ...
                'signal':signal,'sponsor_pixels':sponsor_pixels,'native_light_bits':native_lights,
                'recorded_light_bits':record['native_light_bits'],'lamp_amber_pixels':amber_pixels,'lamp_bright_pixels':bright_lamp_pixels})
            poses.append(replay.qpos.copy()); sponsor_counts.append(sponsor_pixels)
            if output_index % 60 == 0 or args.previews:
                print(json.dumps({'frame':output_index,'total':len(timeline),'camera':name,'source_index':source,'pose_error':error,'sponsor_pixels':sponsor_pixels,'wall_seconds':time.perf_counter()-started}),flush=True)
        if video:
            ...
            for _ in range(credit_frames): video.stdin.write(np.asarray(card).tobytes())
    finally:
        client.set_replayer_time_factor(0.)
        for sensor in sensors: sensor.stop(); sensor.destroy()
    ...
    receipt={'status':'previewed' if args.previews else 'rendered','fps':60,'frames':len(records)+(0 if args.previews else credit_frames),
        'duration_seconds':(len(timeline)+credit_frames)/60,'native_motion_frames':len(records),'same_recorded_episode':True,'edit_plan':edit,'start_alignment':start_alignment,
