...
receipt = {'source_revision':os.environ['FLYHARD_SOURCE_REVISION'],
    ...
    'sharp':subprocess.check_output(['node','-p',"require('/opt/flyhard/node_modules/sharp/package.json').version"],text=True).strip(),
    ...
    'source_sha256':{str(p.relative_to(root)):hashlib.sha256(p.read_bytes()).hexdigest()
        for folder in ['src','scripts','docker','deploy','apps'] for p in sorted((root/folder).rglob('*'))
        if p.is_file() and p.suffix in ['.py','.sh','.mjs']}}
if receipt['runtime_layout'] == 'network-volume':
    receipt['carla_asset'] = 'selected-and-verified-at-startup'
else:
    receipt['carla_archive_sha256'] = '09e3ebb28df17962f0c997e66f4b914ad5ea6f1d6a6dbbf13c9f87eb38346d57'
