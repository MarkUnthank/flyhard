...
- Do not manually publish production Worker code or assets from any checkout, worktree, machine, or agent. This includes `wrangler deploy`, `wrangler versions upload`, `wrangler versions deploy`, OpenNext deploy/upload commands, npm deployment scripts, and equivalent Cloudflare API or dashboard actions. Do not bypass this rule using another tool or credential.
