...
- Checkout asks only for a spot and bid amount. The server stores the immutable price and spot before creating Stripe Checkout; a stable idempotency key makes network retries safe. No brand details or uploads are required to pay. Opening checkout does not reserve a spot.
...
- Confirmed payments enter durable `awaiting_details` state. They do not replace the current advertiser or appear in public history until the buyer returns and publishes their brand, HTTPS website, optional message, artwork, and logo. The private Stripe session returned to the buyer authorizes `/api/checkout/details`; public bid IDs grant no access.
- Publication compares the paid bid to the latest winner and atomically binds the uploads, saves the profile, and commits the replacement, history, and revision. Duplicate detail submissions and payment deliveries cannot publish twice or reduce the price.
- The browser remembers checkout sessions before leaving for Stripe and checks them on return or a later visit. Paid spots reopen in the details editor and remain available through a “Finish spot” button if dismissed. The editor also provides a private completion link. Cancellation returns to the same spot and amount. Session credentials and unfinished profiles are never included in public auction data.
- If an already-paid bid loses before publication, including while the buyer is entering details, the server requests a full Stripe refund with a stable idempotency key. Publishing a higher bid also refunds unfinished lower paid bids. If it was published first and then outbid, it is not refunded.
...
2. The browser also prepares a separate logo thumbnail from the original upload (up to 512 pixels), preserving transparency without applying the car placement's crop, position, or rotation. After payment, the details form collects the advertiser's name, HTTPS website, and optional message of up to 140 characters. The Worker size-checks, decodes, and re-encodes both PNGs, strips metadata, and stores them under immutable random R2 keys **after** payment. Both images stay private until the final details submission binds them to that purchase and publishes them.
3. Once payment is verified and the buyer submits their complete details, the transaction publishes the artwork and logo URLs, name, message, and website together and increments the livery revision. The auction table gives the advertiser the wide first column. Existing purchases receive an empty message and use their previously published artwork as the logo during the storage migration.
...
`npm run test:checkout` starts an isolated local API and Next preview on ports 8799 and 3119, drives desktop/mobile Chromium through bidding, cancellation, payment return, recovery, artwork upload, and publication, and saves screenshots under `artifacts/checkout-flow/`. It uses a fake Stripe provider and blocks external browser requests; no real card payments, production storage, or emails are involved. It stops its services when finished.

The tests run the actual Worker and SQLite/R2 bindings in Miniflare. Stripe HTTP calls are isolated with an in-process fake provider; no tests can contact the real Stripe API. They cover upload privacy, malformed files, hostile origins, forged signatures, unpaid checkouts, competing payments that fail the $1 increment, duplicates, refunds, WebSocket delivery, and immutable artwork serving. Advertiser profiles are checked for message length, immutable checkout retries, private logo uploads, prevention of upload reuse, and publication of both images and the message. The payment-first tests cover amount-only checkout, unpaid and unknown-session rejection, paid-but-unpublished privacy, concurrent detail submissions, lost webhooks, restart recovery, unfinished payments surviving cleanup, and a higher bid winning during the details step.
