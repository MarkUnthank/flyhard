...
import torch
from flyhard.three_point import encode,OBSERVATION_FIELDS
...
def load_policy(checkpoint,graph_dir='data/graph-traced-v1',reset_core=False):
    ...
    s=saved['model'];policy=ThreePointPolicy(np.load(Path(graph_dir)/'graph.npz'),s['sensory_ids'],s['motor_ids'],saved['config']['seed'])
...
def main():
    ...
    take=np.linspace(0,len(val['targets'])-1,min(192,len(val['targets'])),dtype=int)
    ...
    config={**vars(args),'graph_sha256':sha(Path(args.graph)/'graph.npz'),'cases_sha256':sha(root/'cases.json'),
        ...
        'claim':'Short supervised three-point-turn pilot from relative goal and road geometry; physical evaluation required.',
        ...
        'source_sha256':{n:sha(n) for n in ['src/flyhard/three_point.py','src/flyhard/three_point_policy.py','src/flyhard/parking_policy.py','src/flyhard/parking_rig.py',__file__]}}
