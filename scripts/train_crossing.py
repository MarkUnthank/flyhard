#!/usr/bin/env python3
"""Train pedal demands for a pedestrian crossing inside the measured graph.
...
from flyhard.crossing import OBSERVATION_FIELDS, encode
...
def load_policy(checkpoint, graph_dir='data/graph-traced-v1', reset_core=False):
    ...
    s = saved['model']
    policy = CrossingPolicy(np.load(Path(graph_dir)/'graph.npz'), s['sensory_ids'], s['motor_ids'],
                            saved['config']['seed'])
...
def main():
    ...
    if args.resume:
        ...
        policy = CrossingPolicy(np.load(Path(args.graph)/'graph.npz'),
                                np.flatnonzero(classes == 'vnc_sensory'),
                                np.flatnonzero(classes == 'vnc_motor'), args.seed).cuda()
    ...
    config = {**vars(args), 'graph_sha256': sha(Path(args.graph)/'graph.npz'),
              ...
              'claim': 'Supervised pedestrian-crossing pedal control from measured longitudinal '
                       'and pedestrian geometry; native CARLA evaluation required.',
              ...
              'source_sha256': {n: sha(n) for n in ['src/flyhard/crossing.py',
                                                    'src/flyhard/crossing_policy.py',
                                                    'src/flyhard/crossing_teacher.py', __file__]}}
    ...
    (out/'metrics.json').write_text(json.dumps(
        {'status': 'trained; native crossing evaluation pending', 'initial_validation_loss': initial,
