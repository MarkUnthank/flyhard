...
def watch():
    ...
    while True:
        ...
        if state.get("closed"):
            return
        if state.get("guard_pid") not in (None, os.getpid()):
            # A reconciled failed allocation may be replaced before an old
            # watcher wakes. Only the new session's watcher should own it.
            return
        try:
...
def compute_hourly(config):
    """Price one explicitly selected CPU or GPU from the live provider catalog."""
    if bool(config.get("cpu")) == bool(config.get("gpu")):
        raise RuntimeError("Choose exactly one CPU or GPU configuration")
    if config.get("cpu"):
        cpu = config["cpu"]
        count = cpu["vcpuCount"]
        if not isinstance(count, int) or count < 2 or count & (count - 1):
            raise RuntimeError("CPU count must be a power of two, at least two")
        if config.get("cloud", "SECURE") != "SECURE":
            raise RuntimeError("CPU pricing requires SECURE cloud")
        catalog = request("GET", "/v2/catalog/cpus/" + urllib.parse.quote(cpu["id"], safe=""))
        if not catalog["vcpu"]["min"] <= count <= catalog["vcpu"]["max"]:
            raise RuntimeError("CPU count outside provider limits")
        return catalog["price"]["securePerVcpu"] * count
    gpu = config["gpu"]
    if gpu.get("count", 1) != 1:
        raise RuntimeError("Pilot is limited to one GPU")
    catalog = request("GET", "/v2/catalog/gpus/" + urllib.parse.quote(gpu["id"], safe=""))
    return catalog["price"][config.get("cloud", "SECURE").lower()]
...
def launch(config_path):
    ...
    # Runpod's basic SSH gateway uses a separate, Pod-scoped override. It lets
