...
def renewed_arguments(arguments, deadline):
    if 'pod_deadline.py' not in (arguments or ''):
        raise RuntimeError('Resume requires an explicit on-Pod deadline timer in startup arguments')
    updated, count = re.subn(r'(--deadline\s+)[0-9]+(?:\.[0-9]+)?',
                            lambda m: m.group(1)+str(deadline), arguments)
    if count != 1:
        raise RuntimeError('Expected exactly one on-Pod deadline to renew')
    return updated
...
def main():
    ...
    args=p.parse_args()
    if not 120<=args.seconds<=7200 or args.reserve<4 or not 0<args.spend_cap<=6:
        p.error('Use 120–7200 seconds, at least $4 reserve, and a spend cap up to $6')
    ...
    now=time.time()
    startup = renewed_arguments(pod.get('args'), now+args.seconds)
    state={'name':pod['name'],'pod_id':pod['id'],'requested_epoch':now,
    ...
    try:
        control.request('PATCH','/v2/pods/'+pod['id'],{'args':startup})
        checked=control.request('GET','/v2/pods/'+pod['id'])
        assert checked['args']==startup, 'Deadline update did not persist'
        result=control.request('POST','/v2/pods/'+pod['id']+'/action',{'action':'start'})
    except control.RunpodError as exc:
        latest=control.request('GET','/v2/pods/'+pod['id'])
        if 400<=exc.status<500 and latest['status']=='EXITED':state['closed']=True;control.save_state(state)
        raise
