...
from flyhard.horn import HornCase, ObservationHistory, encode_history, score, valid_light_sequence
...
def capture(env, model, saved, args):
    ...
    config = {**vars(args), 'fps':60, 'width':1248, 'height':960, 'fov_degrees':65,
              ...
              'claim':'Learned horn from structured light/car history. Car approach/braking and zero steering request are conventional controls.',
              'clock':'10 Hz neural decisions command seven fly joints at 300 Hz. Physics and CARLA advance 1/60 s together. Sound follows measured button state at displayed frame times.',
              'source_sha256':{p:sha(p) for p in [__file__, 'src/flyhard/horn.py', 'src/flyhard/horn_rig.py', 'src/flyhard/horn_world.py']}}
    ...
    try:
        for i in range(round(args.seconds*60)):
            ...
            if i % 6 == 0:
                ...
                encoded = encode_history(history.update(observed))
                with torch.no_grad():
            ...
            env.set_green(args.kind == 'arrive_green' or t >= args.green_at)
            steer = float(np.clip(rig.angle*1.7, -.85, .85))
            env.ego.apply_control(env.approach_control(t, steer))
            for _ in range(5):
                rig.step_horn(command)
            frame = env.world.tick()
            ...
            row = {'index':i, 'body_index':i, 'neural_index':len(activity)-1,
                   ...
                   'requested_angle':0., 'wheel_angle':rig.angle, 'applied_steer':steer,
    ...
    case = HornCase(args.seed, args.kind, args.gap, observed_green, args.seconds)
    metrics = {'status':'capture_complete', 'frames':len(frames), 'native_fps':60,
               ...
               'score':score(case,[r['camera_time'] for r in frames],[r['horn_pressed'] for r in frames]),
    ...
    valid_light = valid_light_sequence(args.kind, neural_green, displayed_green)
    metrics['scenario_light_sequence_valid'] = valid_light
    if not valid_light:
        metrics['status'] = 'invalid_scenario'
...
def main():
    ...
    p.add_argument('--kind', choices=['wait_then_green','no_car','arrive_green'], required=True)
    p.add_argument('--seconds', type=float, default=8.)
    p.add_argument('--green-at', type=float, default=3.6)
    p.add_argument('--gap', type=float, default=7.2)
    p.add_argument('--seed', type=int, default=140001)
    args = p.parse_args()
    torch.set_num_threads(4)
    model, saved = load_policy(args.checkpoint)
    env = HornWorld()
    try:
