#!/usr/bin/env python3
"""Edit the saved three-point-turn trajectory, body and neural samples into a 60 fps video.

Vehicle transforms are presentation replay, with physics disabled. They are never
used to evaluate the turn or fed back to the controller. No corrections are added.
"""
import argparse
import json
import math
...
from flyhard.parking import rectangle,CAR_LENGTH,CAR_WIDTH
from flyhard.three_point import TurnCase
from evaluate_three_point import native_world
...
def main():
    ...
    p.add_argument('--attempt',type=int,default=1);p.add_argument('--label');p.add_argument('--start',type=float,default=0.);p.add_argument('--duration',type=float);p.add_argument('--rate',type=float,default=1.)
    ...
    env=native_world();env.start(TurnCase(**config['case']))
    if env.scene['map']!=config['map'] or not np.allclose(env.origin,config['origin']):
        env.close();raise RuntimeError('Recorded native scene differs from replay scene')
    settings=env.world.get_settings();settings.no_rendering_mode=False;env.world.apply_settings(settings)
    for a in env.actors:
    ...
    for kind in ['rgb','depth']:
        ...
        for k,v in {'image_size_x':str(W),'image_size_y':str(H),'fov':'60','sensor_tick':'0','lens_k':'0','lens_kcube':'0'}.items():bp.set_attribute(k,v)
    ...
    # Mark the full scored road corridor and the target footprint.
    lines=[]
    for y in [-env.case.width/2,env.case.width/2]:lines.append(((-12,y),(8,y)))
    target=rectangle(env.case.goal_x,env.case.goal_y,np.pi,CAR_LENGTH+1.2,CAR_WIDTH+1.2)
    lines.extend(zip(target,np.roll(target,-1,axis=0)))
    gx,gy=env.case.goal_x,env.case.goal_y
    lines.extend([((gx+1.4,gy),(gx-1.4,gy)),((gx-1.4,gy),(gx-.6,gy-.55)),((gx-1.4,gy),(gx-.6,gy+.55))])
    pieces=[]
    for a,b in lines:
        local=np.asarray(a)[None]+np.linspace(0,1,800)[:,None]*(np.asarray(b)-a)[None]
        for x,y in local:
            loc=env.transform(x,y).location;pieces.append([loc.x,loc.y,.035,1.])
    bay_points=np.asarray(pieces)
    ...
    video=None if args.previews else subprocess.Popen(['ffmpeg','-nostdin','-v','error','-y','-f','rawvideo','-pix_fmt','rgb24',
        ...
        '-pix_fmt','yuv420p','-movflags','+faststart',str(out/'three-point.mp4')],stdin=subprocess.PIPE)
    ...
    duration=args.duration or max(.05,len(frames)*.05-args.start)/args.rate
    total=round(duration*60)
    previews={0,total//5,2*total//5,3*total//5,4*total//5,total-1}
    try:
        # Replay exactly the selected recorded interval; never alter its controls.
        for index in range(total):
            if args.previews and index not in previews:continue
            source=min((args.start+index/60*args.rate)*20,len(frames)-1);lo=int(source);hi=min(lo+1,len(frames)-1);mix=source-lo;r=frames[lo]
            ...
            # Hold wide views long enough to see each change of direction.
            phase=index/max(total-1,1)
            if phase<.40:
                name='wide';position=env.transform(-2,6).location;position.z=13
                target=env.transform(-2,0).location;target.z=.6
                camera=look_at([position.x,position.y,position.z],[target.x,target.y,target.z])
            elif phase<.62:
                name='left_door';relative=look_at([-4,-6,4.2],[0,0,.8])
                camera=transform(matrix@np.asarray(relative.get_matrix()))
            else:
                name='rear_wide';position=env.transform(5,6).location;position.z=11
                target=env.transform(-3,0).location;target.z=.6
                camera=look_at([position.x,position.y,position.z],[target.x,target.y,target.z])
            ...
            focal=W/(2*math.tan(math.radians(60)/2));den=np.maximum(camera_points[:,0],.01)
            ...
            relative=np.linalg.inv(matrix)@np.asarray(camera.get_matrix());raw,pixels=sponsor.render(relative,60,raw,depth)
            ...
            title='flyhard | three-point turn'
            ...
            draw.text((48,980),f"{args.label or ('Attempt '+str(args.attempt))} · {source*.05+.05:.1f}s elapsed",font=font[22],fill='white',stroke_width=2,stroke_fill='black')
            ...
            records.append({'frame':index,'source_index':source,'camera':name,'sponsor_pixels':pixels,'neural_index':r['neural_index'],'body_frame_index':lo,
                            'camera_world_matrix':camera.get_matrix(),'vehicle_matrix':matrix.tolist(),'fov':60,'width':W,'height':H})
    ...
    receipt={'status':'preview' if args.previews else 'rendered','fps':60,'duration':duration,'frames':len(records),
             'presentation_title':'flyhard | three-point turn','hero_color_rgb':[48,84,43],'outcome_caption':False,
             'native_camera_cache':'Unsponsored native RGB and bit-exact packed depth retained for future sponsor-only recomposition.',
             'attempt_number':args.attempt,'stage_label':args.label,'playback_rate':args.rate,'source_start_seconds':args.start,'actual_result':result,'sponsor_revision':manifest['revision'],'sponsor_layout':manifest['layoutVersion'],
             'motion':'Recorded native CARLA rigid poses and MuJoCo joint positions interpolated only for presentation; physics disabled during replay. Cosmetic front-wheel angles use recorded steer times the measured 70 degree limit. Target pose outline includes a 0.6 m display margin and opposite-heading arrow; scored road boundaries are exact. All markings use native depth occlusion.',
    ...
    if video:receipt['video_sha256']=sha(out/'three-point.mp4')
