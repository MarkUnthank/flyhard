"""Pedal demands learned inside the full measured connectome.

Two sigmoid heads produce throttle and brake travel. The teacher never demonstrates
both at once, so separation is learned from the data rather than imposed by the
decoder; a trial that presses both is a real failure and is scored as one.
"""
import numpy as np
import torch

from flyhard.connectome import SparseConnectome
from flyhard.crossing import encode


class CrossingPolicy(torch.nn.Module):
    def __init__(self, graph, sensory_ids, motor_ids, seed=733):
        super().__init__()
        ...
        count = encode(np.zeros((1, 10), np.float32)).shape[1]
        self.register_buffer('feature_ids', torch.as_tensor(rng.integers(0, count, len(sensory_ids)), dtype=torch.long))
        self.register_buffer('input_signs', torch.tensor(rng.choice([-1., 1.], len(sensory_ids)), dtype=torch.float32))
        self.register_buffer('decoder', torch.tensor(rng.normal(size=(2, len(motor_ids)))/np.sqrt(len(motor_ids)),
                                                     dtype=torch.float32))
