"""Learned connectome horn motor control with a frozen raw-history interface."""
import numpy as np
import torch
from torch import nn

from flyhard.connectome import SparseConnectome
from flyhard.horn import FIELDS, HISTORY, encode_history


class HornPolicy(nn.Module):
    def __init__(self, graph, sensory_ids, motor_ids, neutral, action_scale, seed=501):
        super().__init__()
        self.core = SparseConnectome(graph['crow'], graph['col'], graph['counts'])
        rng = np.random.default_rng(seed)
        n_features = len(encode_history(np.zeros((HISTORY, len(FIELDS)))))
        ...
        self.register_buffer('feature_ids', torch.tensor(rng.integers(0, n_features, len(sensory_ids))))
    ...
    def calibrate(self, features):
        with torch.no_grad():
            ...
            self.decoder.mul_((.20 / raw.std(dim=0).clamp_min(1e-5))[:, None])
