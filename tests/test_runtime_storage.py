...
def test_retained_archive_can_restore_a_runtime_without_downloading(tmp_path, monkeypatch):
    seed = load('seed_offline', 'deploy/seed_runtime_volume.py')
    root, downloads = tmp_path/'volume', tmp_path/'downloads'
    root.mkdir(); downloads.mkdir()
    archive = downloads/'fixture.tar.gz'
    with tarfile.open(archive, 'w:gz') as output:
        ...
        member.size = len(data); member.mode = 0o755
        output.addfile(member, io.BytesIO(data))
    spec = {'url': 'https://network-must-not-be-used.invalid/fixture.tar.gz',
            'suffix': '.tar.gz', 'binary': 'simulator.sh', 'sha256': seed.sha256(archive)}
    original_run = seed.subprocess.run
    def offline_run(command, **kwargs):
        assert command[0] == 'tar', 'Restoration must not invoke a downloader'
        return original_run(command, **kwargs)
    monkeypatch.setattr(seed.subprocess, 'run', offline_run)
    first = seed.seed(root, 'fixture', spec, downloads)
    assert archive.is_file()
    shutil.rmtree(root/'fixture')
    restored = seed.seed(root, 'fixture', spec, downloads)
    assert restored['binary_sha256'] == first['binary_sha256']
    assert seed.sha256(archive) == spec['sha256']


def test_native_dependency_cache_reuses_verified_bytes_and_rejects_damage(tmp_path, monkeypatch):
    cache = load('native_archive_cache', 'deploy/native/cache_carla_archive.py')
    monkeypatch.setattr(cache, 'ROOT', tmp_path)
    archive = tmp_path / 'fixture.tar.gz'
    archive.write_bytes(b'verified dependency archive')
    spec = {'name': archive.name, 'bytes': archive.stat().st_size,
            'sha256': cache.sha256(archive), 'url': 'https://unused.invalid/fixture.tar.gz'}
    def no_download(*args, **kwargs):
        raise RuntimeError('Downloader forbidden in offline check')
    monkeypatch.setattr(cache.subprocess, 'run', no_download)
    assert cache.fetch(spec) == archive
    archive.write_bytes(b'damaged dependency archive!')
    with pytest.raises(RuntimeError, match='Downloader forbidden'):
        cache.fetch(spec)
